...
inline void updateAnswer()
{
	...
	{
		using namespace std::chrono;
		if (quest + 600000 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
		{
				...
				++currentPeer)
			{
				if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
					continue;
				Player::OnConsoleMessage(currentPeer, "`9** GrowZS Daily Math : `3'" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "'`9 = ? Prize: `2" + to_string(prize) + "`9 (gems) ! `o(/a <answer>).");;
			}
			cout << " ** GrowZS Daily Math : '" + to_string(resultnbr1) + " + " + to_string(resultnbr2) + "' = ? Prize: " + to_string(prize) + " gems ! `o(/a answer)." << endl;
		}
	}
...
inline void banLoginDevice(ENetPeer* peer, const long long banDurationDefault, string sid, string mac)
{
	...
	const auto text = "action|log\nmsg|`4Sorry, this device or location is still banned for `w" + OutputBanTime(calcBanDuration(banDurationDefault)) + " Need help? Contact Jeo & iLand!";
	const string dc = "https://discord.gg/JWTH8v3w9B";
	const auto url = "action|set_url\nurl|" + dc + "\nlabel|Join GrowZS Discord\n";
...
inline void SearchInventoryItem(ENetPeer* peer, const int fItemid, const int fQuantity, bool& iscontains)
{
	...
	for (auto i = 0; i < static_cast<PlayerInfo*>(peer->data)->inventory.items.size(); i++)
	{
		if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemID == fItemid)
		{
			if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).madeBy != "") history += static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).madeBy;
			break;
		}
	}
...
inline void checkTimeBan(ENetPeer* peer, string username)
{
	...
	if (exist)
	{
		...
		if (fd.fail())
		{
			...
			Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + username + "' is time banned. `5Please send this screenshot to GrowZS developers.");
		...
		if (bantimeleft < 1) return;
		auto text = "action|log\nmsg|`4Sorry, this account is temporary banned by `w" + bannedBy + " `4for `w" + OutputBanTime(calcBanDuration(banDuration)) + " `4for `w" + bannedReason + ". `4Need help? `8Contact Jeo & iLand!";
		string dc = "https://discord.gg/JWTH8v3w9B";
	...
	if (exist)
	{
		...
		if (experimental::filesystem::exists("autoban/ip/" + ip + ".txt"))
		{
			...
			if (ifs.fail())
			{
				...
				Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + static_cast<PlayerInfo*>(peer->data)->rawName + "' is auto banned by system. `5Please send this screenshot to GrowZS developers.");
				if (!static_cast<PlayerInfo*>(peer->data)->Console) enet_peer_disconnect_later(peer, 0);
			}
			else
			{
...
inline void checkIpBan(ENetPeer* peer)
{
	...
	if (exist)
	{
		string text = "action|log\nmsg|`4Sorry, this device or location is perma banned. Need help? Contact `2Jeo & iLand!";
...
inline int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string email,string code)
{
	...
	vector<string> checkip = getregistered["ip"];
		...
		if (ridcount >= 5 || sidcount >=5|| gidcount >=5 || maccount >=5 || vidcount >=5 || ipcount >=5) return -14;
...
{
	if (static_cast<PlayerInfo*>(peer->data)->adminLevel >= 3) return true;
	else return false;
}
...
string getModsOnlineCount()
{
		...
		++currentPeer)
	{
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
			continue;
		if (((PlayerInfo*)(currentPeer->data))->adminLevel >= 4) {
			modCount++;
		}
	}
	if (modCount == 1)
...
inline void GiveRank(string rankName, string playerName, bool& success)
{
		...
		else if (PlayerDB::getProperName(rankName) == "support")
		{
			rankLevel = 3;
		}
		...
		else if (PlayerDB::getProperName(rankName) == "youtube")
		{
			rankLevel = 5;
		}
...
void sendEffectP(ENetPeer* peer, int effect)
{
	((PlayerInfo*)(peer->data))->effect = 8421376 + effect;
	PlayerInfo* info = ((PlayerInfo*)(peer->data));
	int ccc = 8421376;
	int countt = ccc + effect;
	std::string s = std::to_string(countt);
	int netID = info->netID;
	ENetPeer* currentPeer;
	string kek = to_string(effect);
	sendPuncheffectpeer(peer, ((PlayerInfo*)(peer->data))->effect);
	sendPuncheffect(peer, ((PlayerInfo*)(peer->data))->effect);
	sendConsoleMsg(peer, ">> Your `2Punch Effect`` has changed to `w" + kek + "``!");
}
void sendEffectP2(ENetPeer* peer, int effect)
{
	((PlayerInfo*)(peer->data))->effect = 8421376 + effect;
	PlayerInfo* info = ((PlayerInfo*)(peer->data));
	int ccc = 8421376;
...
inline void LoadPlayerData(ENetPeer* peer)
{
	if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
	{
		...
		case 9:
			...
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`4Server-Creator`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 8:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`cOwner`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`cOwner`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 7:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`4SCO`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`4SCO`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 6:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`6Manager`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`6Manager`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 5:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`rGOD`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`rGOD`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 4:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`4Adm`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`4Adm`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 3:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`#Mods`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`#Mods`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 2:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`4You`wTube] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`4You`wTube] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 1:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[`1VIP`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`1VIP`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		case 0:
			static_cast<PlayerInfo*>(peer->data)->displayName = "`w[cGrowZS`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			static_cast<PlayerInfo*>(peer->data)->displayNamebackup = "`w[`cGrowZS`w] " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
			break;
		default:
